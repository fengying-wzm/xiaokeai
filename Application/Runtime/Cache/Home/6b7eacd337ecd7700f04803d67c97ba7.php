		   ...
		   <div class="container">
			   <div class="collapse navbar-collapse" id="example-navbar-collapse">
				  <ul class="nav navbar-nav">
					 ...
					 <li ><a class="current"  href="sex">两性</a></li>
		...
		<div class="row index-row">
			...
			<div class="col-md-4 catalog_left visible-lg">
				<div class="catalog_main_title">
					...
					<div class="catalog_title">
						<img src="/GetHot/Public/images/u34.png" >
					</div>
				</div>
				<div class="btn-group-vertical btn-groups btn-group-lg">
				    <button type="button" class="btn btn-default btn_title">常用标签</button>
					<?php if(is_array($label)): $i = 0; $__LIST__ = array_slice($label,0,null,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo == $_GET['tags']): ?><button type="button" onclick="window.location.href='sex?tags=<?php echo ($vo); ?>';" class="btn btn-default active"><?php echo ($vo); ?></button>
						<?php else: ?> 
				    		<button type="button" onclick="window.location.href='sex?tags=<?php echo ($vo); ?>';"  class="btn btn-default"><?php echo ($vo); ?></button><?php endif; endforeach; endif; else: echo "" ;endif; ?>
			...
			<div class="col-md-7 ">
				<div class="catalog_article">
					...
					<?php if(is_array($articleList)): $i = 0; $__LIST__ = array_slice($articleList,0,15,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="catalog" onclick="window.location.href='sex_detail?id=<?php echo ($vo["id"]); ?>';"><span><?php echo ($vo["title"]); ?></span><span class="all"><?php echo ($vo["author"]); ?></span></div><?php endforeach; endif; else: echo "" ;endif; ?>
    ...
    <script src="/GetHot/Public/js/bootstrap.min.js"></script><script>
		...
		var isLoading = false;
			...
			function load () {
				var couponList;
				$.ajax({
					url : '/GetHot/index.php/Home/Index/sex_list',
					type : 'post',
					cache : false,
					dataType : 'json',
					data:{
					...
					success : function(object) {
						if(object.reCode=="0"){
							...
							$.each(object.list,function(idx,item){
								...
								coupon_list = '<div class="catalog" onclick="window.location.href=\'sex_detail?id='+id+'\';"><span>'+title;
